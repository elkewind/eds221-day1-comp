...
# %>% = "and then" = |> (native pipe)
# Use ctr + shift + m to get pipe
...
```{r}
toolik_temp_plot <- ggplot(data = toolik_weather, aes(x = as.factor(date), y = daily_air_temp_mean_c)) +
...
```{r}
ggsave(here::here("figs", "toolik_temp_plot_example.png"), height = 6, width = 7)
```
